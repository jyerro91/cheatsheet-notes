...
7. Volumes
  ...
  - *Note: K8s doesnt manage data persistence*
8. Deployment
  - Blueprint for apps pods
  - Abstraction of pods
  - Ideal for stateless apps
  - *Note: DB can't be replicated via deployment*
9.  Statefulset
...
### Main Kubectl Commands
    ...
    - Update deployment                                         kubectl edit deployment $NAME
    - Delete deployment                                         kubectl delete deployment $NAME
    - Delete all resources in the default namespace             kubectl delete all --all
    - Delete all from certain namespace                         kubectl delete all --all -n $NAMESPACE
    - Delete and Recreate from certain namespace                kubectl delete namespace $NAMESPACE; kubectl create namespace $NAMESPACE
                                                                or kubectl delete "$(kubectl api-resources --namespaced=true --verbs=delete -o name | tr "\n" "," | sed -e 's/,$//')" --all


    ### Debugging Pods
    - Log to console                                            kubectl logs $PODNAME
    - Get interactive terminal                                  kubectl exec -it $PODNAME -- /bin/bash
    - Get info about pod                                        kubectl describe pod $PODNAME
    - Get more info about pod                                   kubectl get pod -o wide
    - Get status of diff components                             kubectl get {nodes,pod,services,deployment,replicaset}
    - Get latest status from etcd                               kubectl get deployment $DEPLOYMENT_NAME -o yaml > filename.yml

    ### Use conf file for CRUD
    - Apply a configuration file                                kubectl apply -f $MANIFEST_FILE/$MANIFEST_FOLDER_PATH
    - Delete a configuration file                               kubectl delete -f $MANIFEST_FILE/$MANIFEST_FOLDER_PATH

   #### To create a deployment
   **kubectl create deployment $NAME --name=$IMAGE**
   - blueprint for creating pods
   - most basic configuration for deployment
   - rest defaults

  `Note: Layers of Abstraction`
